...
def vec_or(v1,v2):
    ...
    res=v1+v2
    #print(res)
    for i in range(40*(round_number+1)):
        if(res[0][i]>=1):
            res[0][i]=1
    return res
...
def generate_list_equ(L_mat,var_list):#L_mat size(n,32*round_number+16)
    tmp=np.zeros((1,40*(round_number+1)),dtype=int)
    ...
    list_equ=np.zeros((1,40*(round_number+1)),dtype=int)
    rhs=[]
    flag=True
    print(ind_list)
    for i in var_list:
        line=np.zeros((1,40*(round_number+1)),dtype=int)
