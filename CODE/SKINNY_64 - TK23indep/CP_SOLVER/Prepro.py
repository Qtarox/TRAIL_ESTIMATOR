...
def show_constraint_set(nmat,equ_dic,nlmat):
    ...
    for i in range(np.shape(nlmat)[0]):
        ...
        for k in range(8*round_num):
            if(nlmat[i][k+round_num*32+32]==1):
                str_tmp=str_tmp+' + k_'+str(k)
        str_tmp=str_tmp+'= 0 '
        print(str_tmp)
        equ_LST.append(str_tmp)
...
def CONS_ES(file_path,mat_name,equ_ind_lst):
    ...
    get_key(new_mat,len_lin,Z_list,ARR_X,equ_var)
...
def get_key(c_mat,len_lin,Z_list,ARR_X,equ_var):
    exp_t=1<<15

    for t in range(exp_t):
        random_X=get_random_set(ARR_X)
        KEY_LST=[]
        for i in range(len_lin):
            #view each equation
            equ_ind=equ_var[i]
            ind0=Z_list.index(equ_ind[0])
            row_val=random_X[ind0]
            for j in range(1,len(equ_ind)):
                row_val+=random_X[Z_list.index(equ_ind[j])]
            KEY_LST.append(row_val)

            # if(equ_ind>32*(round_num+1)):#is key
            #     continue
...
if __name__=="__main__":
    ...
    for ind in range(10):
        ...
        # print(len_lin)
        nmat,nl_mat=Prepross_eq(tstm,len_lin,RES_DDT_6[ind])
