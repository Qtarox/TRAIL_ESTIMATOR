...
from TOOL.BASIC_OP import load_dic,list_xor,corr_ind, is_active
...
from LBLOCK_SHOW import show_cons_set
from CONS_SOLVE.Merge import  generate_solu_set#, MAT_XOR
...
def vec_or(v1,v2):
    res=np.zeros(32*(round_number+1))
    res=v1+v2
    #print(res)
        ...
        for j in range(32*(round_number+1)):
            ...
            if(flag==False and list_equ[i][j]==1):
                ...
                if(ind<16):
                    ...
                    l_tmp=l_tmp+" + y_"+str(int(rn))+"_"+str(ind-16)
        for k in range(8*(round_number+1)):
            if(list_equ[i][24*(round_number+1)+k]==1):
...
def check_active(tmp):# check the active S-box related varibles
    ...
    for i in range(32*(round_number+1)):
        if(tmp[0][i]==1):
            ...
            rn=i//24#IN EACH ROUND, WE SAVE X,Y IN A 32-BITS UNIT
            ind=i%24
            ...
            if(ind<16):
                ...
                if('y_'+str(rn)+'_'+str(ind-16) in y_dic):
                    ...
                    list.append(i)
    for i in range(8*(round_number+1)):
        if(tmp[0][i+24*(round_number+1)]==1):
            print("k_"+str(i))
...
def equ_to_matB(ACTIVE_NUM,rhs_list,inactive_flag=0):
    ...
    if(ind<8): #x0-7
        ...
        for i in range(l):
            tmp=np.zeros((1,2),dtype=int)
            tmp[0][1]=rhs_list[i]
            tmp[0][0]=Sbox_inv[ind-16][rhs_list[i]]
            res=np.vstack([res,tmp])  
    return res
...
def generate_list_equ(L_mat,var_list):#L_mat size(n,32*round_number+32)
    tmp=np.zeros((1,32*(round_number+1)),dtype=int)
    ...
    list_equ=np.zeros((1,32*(round_number+1)),dtype=int)
    rhs=[]
    flag=True
    print(ind_list)
    for i in var_list:
        line=np.zeros((1,32*(round_number+1)),dtype=int)
        
        line[0][i]=1
        if(is_active(i,x_dic,y_dic)):
            ind=i%24
            rn=i//24
            if(ind<16):#x_r_i
...
def get_var(mat):#find element in the equation set
    ...
    for i in range(np.shape(mat)[0]):
        tmp=tmp+mat[i]
    var=[]
    for j in range(24*(round_number+1)):
        if(tmp[0][j]>0):
            var.append(j)
    return var
...
def solve_cluster2(MAT_AB,list_mat,linear_mat,list_rhs,linear_rhs,list_active,linear=False):
    ...
    for i in range(np.shape(Final_mat)[1]):
        if(Final_mat[0][i]>=24*(round_number+1)):
    ...
    print("key shape: ",np.shape(key_rows))
    # print(key_rows)
...
if __name__=="__main__":
    ...
    # RESDC3=[[107, 117], [61, 71, 95, 97],  [62, 64, 88, 98], [28, 38, 44, 54]]
    RESDC3=[[12, 22, 28, 38]]
    RESDC1=[[9, 19, 26, 36, 44, 45, 54, 55], [46, 48], [29, 39], [28, 38]]
    ...
    for i in range(len(gmat_res)):
        ...
        np.save(file_pth+'/tst_mat'+str(i)+'.npy',chosen_mat)
